...
export function RealtimeScreen({ onDataChange }: { onDataChange?: (data: any, error: string | null) => void }) {
  ...
  const startStreaming = async () => {
    try {
      ...
      console.error = (msg, ...args) => {
        ...
        if (
          typeof msg === "string" &&
          (msg.includes("failed to fetch settings") || msg.includes("ERR_CONNECTION_REFUSED"))
        ) {
      ...
      const errorMessage =
        error instanceof Error ? `Failed to stream vision: ${error.message}` : "Failed to stream vision"
  ...
  }, []) // Removed stopStreaming from the dependency array
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          >
            <span>OCR</span>
            <div
              className={`ml-2 w-8 h-4 rounded-full border transition-colors ${withOcr ? "bg-black border-black" : "bg-gray-100 border-gray-300"}`}
            >
              <div
                className={`absolute w-3 h-3 rounded-full transition-transform ${withOcr ? "bg-white translate-x-4" : "bg-white translate-x-1"}`}
              ></div>
            </div>
          </Button>
          ...
          >
            <span>Images</span>
            <div
              className={`ml-2 w-8 h-4 rounded-full border transition-colors ${withImages ? "bg-black border-black" : "bg-gray-100 border-gray-300"}`}
            >
              <div
                className={`absolute w-3 h-3 rounded-full transition-transform ${withImages ? "bg-white translate-x-4" : "bg-white translate-x-1"}`}
              ></div>
            </div>
          </Button>
        </div>
      ...
      <div className="flex items-center gap-1.5 text-right justify-end">
        ...
        <span className="text-xs text-gray-500 font-mono">{isStreaming ? "streaming" : "stopped"}</span>
      </div>
    </div>
  )
}
